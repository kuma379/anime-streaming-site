...
function extractList(data: any): AnimeItem[] {
  if (!data) return [];
  for (const key of ["animeList", "data", "results"]) {
...
export default function GenreDetail() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    ...
    queryFn: () => fetchGenreAnime(genre, page),
    enabled: !!genre,
  });
    ...
    <div className="min-h-screen bg-[#0d0d14]">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-black text-white">Genre: {label}</h1>
            <p className="text-sm text-gray-500">Anime bergenre {label}</p>
          </div>
        </div>
